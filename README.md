...
Our first custom HIPAA-compliant AI agent for customer engagement. The goal for this agent is to have a structured conversation on behalf of Company X with its customers. Company X has provided proprietary data to Flexbone to refer to when answering questions. Flexbone creates custom embeddings using this data for Rosalinda to integrate with. This can contain customer metadata, FAQ transcripts on how to answer certain questions, and other. Rosalinda stores transcripts of all calls in our databases to feed back to company X.
...
1. Start ngrok Locally on port 500X
```
ngrok http 500X
```

2. Copy Forwarding Server Domain to .env (without https://), enter API keys into .env

3. Copy fully qualified forwarding URL and API endpoint to Twilio Console 
a) https://console.twilio.com/
b) Active Numbers -> select number
c) Configure
d) A call comes in -> webhook
e) Set URL to https://ngrok-server-domain.app/call/incoming for example

4. Run application (same port as ngrok)
```
pnpm run dev
```
5. You can now test this by calling the active number under which this was configured
